...
include("../res/bd.php");
$id_aluno = filter_input(INPUT_POST,'uid');
$date = date("Y-m-d H:i:s");
$id_comunidade = filter_input(INPUT_POST,'id');
$photo_url = $target_file;
$txlegenda = filter_input(INPUT_POST,'txlegenda');
$search = "INSERT INTO comunidade_galeria (`id_comunidade`,`id_aluno`, `dtpublicacao`, `txlegenda`, `photo_url`) VALUES ('$id_comunidade','$id_aluno','$date','$txlegenda','handler/$target_file')";
...
if($uploadOk == 0) {
    $pathmod = "?error=".$error."&id=".$id_comunidade;
} else {
    $pathmod = "?id=".$id_comunidade;
}
header("Location: ../comunity-pictures.php".$pathmod);
